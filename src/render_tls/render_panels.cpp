...
{
    short nbuf = 0;
    for (auto i = ptr_app->buf_panels.begin(); i != ptr_app->buf_panels.end(); ++i)
    {
        (*i).nbuf = nbuf;
        nbuf++;
    }
...
{
    short color = (pl.nbuf == ptr_app->active_panel) ? COLOR_PAIR_ACT_PANEL : COLOR_PAIR_FRAME;
    clear_area(ptr_app->back_buf, pl.get_panel(), color);
    frame(ptr_app->back_buf, FRAME, pl.get_panel(), color);
    std::string title = " "+std::to_string(pl.nbuf)+" ";
